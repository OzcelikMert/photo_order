...
namespace AppLibrary
{
    public class ImageResizing : IImageResizing, IDisposable
    {
        ...
        public ImageResizing(string sourceImagePath)
            : this(new MemoryStream(System.IO.File.ReadAllBytes(sourceImagePath)))
        ...
        {
            _jpegEncoder.Rotation = Image.autoRotate(_sourceImagePath);
        ...
        private static BitmapFrame Resize(BitmapFrame bitmapFrame, int width, int height)
        {
            ...
            width = 0;
            height = 0;
            if (bitmapFrame.PixelWidth > bitmapFrame.PixelHeight) width = 1920;
            else height = 1920;
